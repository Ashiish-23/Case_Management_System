    ...
    <div className="bg-slate-900 flex items-center justify-center min-h-screen p-4">
      <div className="max-w-md w-full bg-slate-800 border border-slate-700 rounded-2xl shadow-2xl p-8">
        ...
        <form className="space-y-6" onSubmit={submit}>
          <input type="email" placeholder="officer@police.gov.in" value={email} onChange={e => setEmail(e.target.value)} required className={inputStyle} />

          <button disabled={loading} className="w-full bg-blue-600 py-3 rounded text-white">
            {loading ? "Sending..." : "Send Reset Link"}
          </button>

          <button type="button" onClick={() => navigate("/login")} className="w-full text-white text-sm" >
            ← Return to Login
          </button>
        </form>
      </div>
