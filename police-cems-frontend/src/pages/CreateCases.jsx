...
export default function CreateCase() {
  ...
  const inputStyle = "w-full bg-slate-900 border border-slate-700 rounded-lg px-4 py-3 text-white placeholder-slate-500 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
  return (
    <div className="min-h-screen bg-blue-900 py-10 px-4 flex justify-center items-start">
      <div className="max-w-4xl w-full bg-slate-800 border border-slate-700 rounded-2xl shadow-2xl overflow-hidden">
        <div className="bg-slate-900/50 px-8 py-6 border-b border-slate-700 text-center">
          <h1 className="text-3xl font-bold text-white"> Case Registration Form </h1>
        </div>
        ...
        <form className="p-8 space-y-6" onSubmit={submit}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <input name="caseTitle" maxLength={150} onChange={change} required className={inputStyle} />
            </div>

            <div>
              <label className={labelStyle}>Case Type *</label>
              <select name="caseType" onChange={change} required className={inputStyle} >
          ...
          <div>
            ...
            <textarea name="description" rows="4" maxLength={2000} onChange={change} required className={inputStyle} />
          </div>
          ...
          <div className="pt-6 border-t border-slate-700 flex justify-end gap-4">
            <button type="button" disabled={loading} onClick={() => navigate("/dashboard")} className="px-6 py-3 border border-slate-600 text-white rounded-lg"
            > Cancel </button>

            <button type="submit" disabled={loading} className="bg-blue-600 px-8 py-3 rounded-lg text-white disabled:opacity-50"
            > {loading ? "Submitting..." : "Register Case"} </button>
          </div>
        </form>
      </div>
    </div>
