    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex justify-center items-center z-50 p-4">
      ...
      <div className="bg-slate-800 border border-slate-700 rounded-xl shadow-2xl w-full max-w-lg relative animate-[fadeIn_0.2s_ease-out]">
        ...
        <div className="bg-slate-900/50 px-6 py-4 border-b border-slate-700 flex justify-between items-center">
          ...
          <button onClick={onClose} className="text-white hover:text-white transition-colors"> ✕ </button>
        </div>
        {/* Form Body */}
        <form onSubmit={submit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <textarea value={description} onChange={e => setDescription(e.target.value)} className={`${inputStyle} h-24 resize-none`}
              placeholder="Describe the item securely..." required />
          </div>
          ...
          <div>
            ...
            <input value={category} onChange={e => setCategory(e.target.value)} className={inputStyle}
              placeholder="e.g. Weapon, Narcotic, Document" required />
          </div>
          ...
          <div>
            ...
            <input type="file"  onChange={e => setImage(e.target.files[0])} 
              className="block w-full text-sm text-white file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-semibold
                file:bg-blue-600 file:text-white file:cursor-pointer hover:file:bg-blue-500 hover:file:cursor-pointer "/>
          </div>
          {/* Action Buttons */}
          <div className="flex justify-end gap-3 pt-4 border-t border-slate-700 mt-4">
            <button type="button" onClick={onClose} className="px-4 py-2 rounded-lg text-white hover:text-white hover:bg-slate-700 transition-colors text-sm font-medium"
            > Cancel </button>
            <button type="submit"
