    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex justify-center items-center z-50 p-4">
      <div className="bg-slate-800 border border-slate-700 rounded-xl shadow-2xl w-full max-w-lg relative flex flex-col max-h-[90vh]">
        ...
        <div className="bg-zinc-950/50 px-6 py-4 border-b border-slate-700 flex justify-between items-center shrink-0">
          ...
          <button onClick={close} className="text-white hover:bg-slate-700 p-1 rounded-md" aria-label="Close evidence details" > ✕ </button>
        </div>
        {/* BODY */}
        <div className="p-6 space-y-4 overflow-y-auto">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <div className={`${valueBoxStyle} font-mono text-blue-400`}> {safeText(data.evidence_code)} </div>
            </div>

            <div>
              <label className={labelStyle}>Category</label>
              <div className={valueBoxStyle}> {safeText(data.category || "Uncategorized")} </div>
            </div>
          </div>
          {/* DESCRIPTION */}
          <div>
            <label className={labelStyle}>Description</label>
            <div className={`${valueBoxStyle} min-h-[80px]`}> {safeText(data.description)} </div>
          </div>
          {/* META */}
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className={labelStyle}>Logged By</label>
              <div className={valueBoxStyle}> {safeText(data.logged_by || "Unknown Officer")} </div>
            </div>

            <div>
              <label className={labelStyle}>Date Logged</label>
              <div className={valueBoxStyle}> {safeDate(data.logged_at)} </div>
            </div>
          </div>
          {/* IMAGE */}
          {safeImageUrl && (
            <div>
              ...
              <div className="mt-1 rounded-lg border border-slate-700 bg-black/40 p-1">
                <img src={safeImageUrl} alt="Evidence Proof" className="w-full h-auto max-h-64 object-contain rounded" loading="lazy" 
